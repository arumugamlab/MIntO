...
# '''
# Generate function expression profile from gene-based mode gene profiles
#
...
identity <- args[7]
annot_file <- args[8] #"/emc/cbmr/data/microbiome/processed/SHIME/Cdiff1/DB
...
if (omics == 'metaG_metaT'){
  print('#################################### DB profiles ####################################')
  # Read gene abundances profile
  tpm_profile_df = as.data.frame(fread(profiles_tpm, header=T), stringsAsFactors = F)
  names(tpm_profile_df)[[1]] <- 'ID_gene'
  
  # Annotation ####
  profiles_annot_df <- as.data.frame(fread(annot_file,header=T), stringsAsFactors = F, row.names = T)
  names(profiles_annot_df)[[1]] <- 'ID_gene'
  profiles_annot_df <- subset(profiles_annot_df, select=unique(c("ID_gene", annot_names)))
  
  #### Bed file + Annotations ####
  profiles_annot_tpm_df <- merge(profiles_annot_df, tpm_profile_df, by='ID_gene', all.y=T)
  rownames(profiles_annot_tpm_df) <- profiles_annot_tpm_df$ID_gene
  
  names_list <- names(profiles_annot_df[2:ncol(profiles_annot_df)])
  profiles_annot_tpm_df_sub <- subset(profiles_annot_tpm_df, select= c('ID_gene',names_list))#"KEGG_ko","kofam_KO"
  
  # Split df into metaG and metaT ####
  tpm_profile <- tpm_profile_df
  rownames(tpm_profile) <- tpm_profile$ID_gene
  tpm_profile$ID_gene <- NULL
  ...
  # Delete data
  rm(profiles_annot_df, metaG_tpm_profile, metaT_tpm_profile, tpm_profile)
  ...
  #### phyloseq object
  gene_expression_tpm_nonbinary_phyloseq <- phyloseq(otu_table(as.matrix(gene_expression_tpm_noinf), taxa_are_rows = T), tax_table(as.matrix(profiles_annot_sub)), samp)

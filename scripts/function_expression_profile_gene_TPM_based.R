...
# '''
# Generate function expression profile from gene-based mode gene profiles (TPM normalized)
#
...
dir_DB <- paste0(wd, "/output/data_integration/", map_reference)#, '_GMGC')
input_file <- paste0(dir_DB, "/",omics,".genes_abundances.p", identity, '.',normalization, ".csv")#,".short.csv")
...
if (omics == 'metaG_metaT'){
  ...
  for(db in colnames(profiles_annot_norm_df_sub)[3:mappings_ncol]){#change it deppending on the number of selectedMappings columns
    print(db)
    message(Sys.time(), ": Running ", db)
    dbMap <- profiles_annot_norm_df_sub[,c("coord", "ID_gene",db)]
    ...
    singleKeys <- strsplit(dbMap[,3], split = "\\; |\\;|\\, |\\, |\\||\\| ")
    db_name <- gsub(' ', '.', db)
    if (length(singleKeys) > 0){
      ...
      keyMap_u <- keyMap[!duplicated(keyMap[c(1,2,3)]),]
      ...
      if (db_name %like% 'eggNOG.OGs'){
        ...
        annot_df <- merge(keyMap_funct, test, by.x='Funct' , by.y='Pfam_id', all.x=T)
        rm(test, x, xx)
      } else if (db_name %like% paste(c('dbCAN', 'CAZy'), collapse='|')){
        x <- PFAMCAZY
        mapped_keys <- mappedkeys(x)
        xx <- as.list(x[mapped_keys])
        test <- as.data.frame(do.call(rbind, xx))
        test$Pfam_id <- rownames(test)
        test_cazy <- as.data.frame(test %>% 
                                     dplyr::group_by(Pfam_id) %>% 
                                     dplyr::mutate(CAZY_id = paste(unique(c(V1, V2, V3, V4)), collapse = ',')))
        ...
        annot_df <- as.data.frame(keyMap_funct_desc %>%
                        dplyr::group_by(Funct)%>%
                        dplyr::summarise(Description = paste(unique(Description), collapse=';')))
        annot_df$Description[annot_df$Description=='NA'] <- NA
        rm(test, test2, test3, test_cazy, test_cazy.keyMap, test_cazy.singleKeys, x, xx, keyMap_funct_desc)
      } else {
  ...
  for(db in colnames(profiles_annot_norm_df_sub)[3:mappings_ncol]){#change it deppending on the number of selectedMappings columns
    ...
    if (length(singleKeys) > 0){
      ...
      # Delete data
      rm(omics_function_expression_noinf_plot, omics_function_expression_noinf, omics_funct_profile_rowsum_not_0_annot_dplyr)
    } else{
      # create .csv, .rds, .pdf
      distance_lab='bray'
      file.create(paste0(integration_dir, '/FA.', db_name ,'.csv'))
